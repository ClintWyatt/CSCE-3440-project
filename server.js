...
app.post('/register', function(request, response) {
	...
	sql = 'CREATE TABLE if not exists virus(virusName varchar(50) not null, infectionRate int, deathRate int,' 
...
app.post('/virusData', function(request, response) {
	...
	connection.query('SELECT * FROM virus WHERE virusName = ? AND userName = ?', [ diseaseName, user ], function(error, results, fields) {
		...
		if (results.length > 0) {
			...
			console.log(user);
			var sql = "INSERT INTO virus (virusName, infectionRate, deathRate, threshold, username, weeks) VALUES('" + diseaseName + "','" + infectionRate + "','" + deathRate + "', '" +
				threshold + "', '" + user + "', '" + weeks + "')";
			connection.query(sql, function(err, result) {
				if (err) {
					throw err;
				}
